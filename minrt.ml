...
let screen = Array.make 3 0.000000 in
let vp = Array.make 3 0.000000 in
let view = Array.make 3 0.000000 in
let light = Array.make 3 0.000000 in
let cos_v = Array.make 2 0.000000 in
let sin_v = Array.make 2 0.000000 in
...
let temp = Array.make 14 0.000000 in
let cs_temp = Array.make 16 0.000000 in
let solver_dist = Array.make 1 0.000000 in
let vscan = Array.make 3 0.000000 in
let intsec_rectside = Array.make 1 0 in
let tmin = Array.make 1 1000000000.000000 in
let crashed_point = Array.make 3 0.000000 in
let crashed_object = Array.make 1 0 in
let end_flag = Array.make 1 false in
let viewpoint = Array.make 3 0.000000 in
let nvector = Array.make 3 0.000000 in
let rgb = Array.make 3 0.000000 in
let texture_color = Array.make 3 0.000000 in
let solver_w_vec = Array.make 3 0.000000 in
let chkinside_p = Array.make 3 0.000000 in
let isoutside_q = Array.make 3 0.000000 in
let nvector_w = Array.make 3 0.000000 in
let scan_d = Array.make 1 0.000000 in
let scan_offset = Array.make 1 0.000000 in
let scan_sscany = Array.make 1 0.000000 in
let scan_met1 = Array.make 1 0.000000 in
let wscan = Array.make 3 0.000000 in
...
let rec read_environ (_ : unit) : unit =
  ...
  let _ = vp.(0) <- cos_v.(0) *. sin_v.(1) *. -200.000000 in
  let _ = vp.(1) <- -.sin_v.(0) *. -200.000000 in
  let _ = vp.(2) <- cos_v.(0) *. cos_v.(1) *. -200.000000 in
...
let rec tracer (viewpoint : float array) (vscan : float array) : bool =
  let _ = tmin.(0) <- 1000000000.000000 in
...
let rec get_nvector_rect (_ : unit) : unit =
  ...
  if rectside = 1 then
    ...
    let _ = nvector.(1) <- 0.000000 in
    nvector.(2) <- 0.000000
  else if rectside = 2 then
    let _ = nvector.(0) <- 0.000000 in
...
let rec utexture
  ...
  if m_tex = 1 then
    ...
    let flag1 =
      let d1 = floor (w1 *. 0.050000) *. 20.000000 in
      if not (10.000000 <= w1 -. d1) then true else false
    in
    let w3 = p.(2) -. o_param_z m in
    let flag2 =
      let d2 = floor (w3 *. 0.050000) *. 20.000000 in
    ...
    let w1 = p.(0) -. o_param_x m in
    let w3 = p.(2) -. o_param_z m in
    let w2 = sqrt (fsqr w1 +. fsqr w3) /. 10.000000 in
  ...
  if crashed_p then
    ...
    let bright =
      if shadow_check_one_or_matrix 0 or_net.(0) crashed_point then 0.000000
        ...
        let br1 = if not (0.000000 <= br) then 0.200000 else br +. 0.200000 in
...
let rec scan_point (scanx : int) : unit =
  if size.(0) <= scanx then ()
    ...
    let _ = vscan.(0) <- (sscanx *. cos_v.(1)) +. wscan.(0) in
    let _ = vscan.(1) <- (scan_sscany.(0) *. cos_v.(0)) -. vp.(1) in
    let _ = vscan.(2) <- (-.sscanx *. sin_v.(1)) +. wscan.(2) in
    let metric = sqrt (fsqr sscanx +. scan_met1.(0)) in
    let _ = vscan.(0) <- vscan.(0) /. metric in
    let _ = vscan.(1) <- vscan.(1) /. metric in
    let _ = vscan.(2) <- vscan.(2) /. metric in
    let _ = viewpoint.(0) <- view.(0) in
    let _ = viewpoint.(1) <- view.(1) in
    let _ = viewpoint.(2) <- view.(2) in
    let _ = rgb.(0) <- 0.000000 in
    let _ = rgb.(1) <- 0.000000 in
    let _ = rgb.(2) <- 0.000000 in
    let _ = raytracing 0 1.000000 in
    let _ = write_rgb () in
    scan_point (scanx + 1)
in
let rec scan_line (scany : int) : unit =
  if not (size.(0) <= scany) then
    let _ =
      scan_sscany.(0) <-
        (let t = scan_offset.(0) -. 1.000000 -. float_of_int scany in
         scan_d.(0) *. t)
    in
    let _ = scan_met1.(0) <- fsqr scan_sscany.(0) +. 40000.000000 in
    let t1 = scan_sscany.(0) *. sin_v.(0) in
    let _ = wscan.(0) <- (t1 *. sin_v.(1)) -. vp.(0) in
    let _ = wscan.(2) <- (t1 *. cos_v.(1)) -. vp.(2) in
    let _ = scan_point 0 in
    scan_line (scany + 1)
  else ()
in
let rec scan_start (_ : unit) : unit =
  let _ = write_ppm_header () in
  let sizex = float_of_int size.(0) in
  let _ = scan_d.(0) <- 128.000000 /. sizex in
  let _ = scan_offset.(0) <- sizex /. 2.000000 in
  scan_line 0
in
let rec rt (size_x : int) (size_y : int) (debug_p : bool) : unit =
  let _ = size.(0) <- size_x in
  let _ = size.(1) <- size_y in
  let _ = dbg.(0) <- debug_p in
